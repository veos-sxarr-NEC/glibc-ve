...
_dl_non_dynamic_init (void)
{
  ...
  _dl_verbose = *(getenv ("VE_LD_WARN") ?: "") == '\0' ? 0 : 1;
  ...
  /* Initialize the data structures for the search paths for shared
     objects.  */
  _dl_init_paths (getenv ("VE_LD_LIBRARY_PATH"));

  /* Remember the last search directory added at startup.  */
  _dl_init_all_dirs = GL(dl_all_dirs);

  _dl_lazy = *(getenv ("VE_LD_BIND_NOW") ?: "") == '\0';

  _dl_bind_not = *(getenv ("VE_LD_BIND_NOT") ?: "") != '\0';

  _dl_dynamic_weak = *(getenv ("VE_LD_DYNAMIC_WEAK") ?: "") == '\0';

  _dl_profile_output = getenv ("VE_LD_PROFILE_OUTPUT");
