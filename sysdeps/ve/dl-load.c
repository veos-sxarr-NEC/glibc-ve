...
/* Used to compare with size(256B) and flags */
#define VE3_DIFF_SECT 0x100
#define VE_AX 0x6
#define VE_WA 0x3
#define VE_WAT 0x403
...
static void
ve_alter_search_path(char *search_path, const char*sep)
{
  char *result_path=NULL;       /* For allocating buffer */
  char *ep = NULL;              /* Raw exclude path */
  char real_ep[PATH_MAX] = {0}; /* Resolved excllude path */
  char *sp = NULL;              /* Raw Search path */
  char real_sp[PATH_MAX]= {0};  /* Resolved search path */
  char *init_sp = NULL;         /* For Initial search path pointer */
  ...
  if(result_path == NULL) /* ENOMEM */
    {
      _dl_signal_error (ENOMEM, NULL, NULL,
		      N_("cannot allocate memory for alter path"));
      return;
    }
  ...
  while (( ep = __strsep (&exclude_path, sep)) != NULL)
    {
      ...
      while (( sp = __strsep (&search_path, sep)) != NULL)
      {
        get_realpath(sp, real_sp);
	//_dl_debug_printf("\nSearch_PATH=%s",real_sp);
	if(strcmp(real_ep,real_sp) != 0)
	{
	  /* Copy the path to buffer */
	  strcat(result_path,sp);
	  /* Concatenate ":" only if already not present at the end of string */
	  if(result_path[strlen(result_path)-1] != ':')
	  strcat(result_path,":");
	}
      }
      /* Replace ":" with "\0" if ":" present at the end of string */
       if(result_path[strlen(result_path)-1] == ':')
        result_path[strlen(result_path)-1] = '\0';
      //_dl_debug_printf("\nResult path=%s",result_path);
      /* Create a duplicate which is the new search path now */
      strcpy(init_sp,result_path);
      search_path = init_sp;
      //_dl_debug_printf("\nNew Search path=%s",search_path);
      result_path[0]='\0';
    }
  /* Release and return */
  free(result_path);
...
_dl_init_paths (const char *llp)
{
  ...
  if(ld_exclude_path != NULL && *ld_exclude_path != '\0')
    {
      strp_dup = strdupa(strp);
      //_dl_debug_printf("\nDefault System Path: %s\n",strp_dup);
      /* Alter the path to exclude the list in Default Path */
      ve_alter_search_path(strp_dup,":;");
      //_dl_debug_printf("\nDefault System Altered Path: %s\n",strp_dup);
      if(strp_dup[0] == '\0' || (strcmp(strp_dup,strp) != 0))
        {
          _dl_signal_error (0, strp_dup, NULL,
                            N_("cannot specify system path in VE3_LD_LIBRARY_EXCLUDE_PATH environment variable"));
        }
    }
  ...
  *aelem = NULL;
  ...
  if (l != NULL)
	...
	{
	  rpath_tmp = (const void *) (D_PTR (l, l_info[DT_STRTAB])
			  + l->l_info[DT_RUNPATH]->d_un.d_val);
	  rpath_tmp2 = rpath_tmp;
	  if(rpath_tmp != NULL && *rpath_tmp != '\0')
	    {
	      rpath_tmp2 = strdupa(rpath_tmp);
	    }
	  ...
	  if(ld_exclude_path != NULL && *ld_exclude_path != '\0')
	    {
	      //_dl_debug_printf("\nOriginal Path: %s\n",rpath_tmp2);
	      /* Alter the path to exclude the list in RUNPATH */
	      ve_alter_search_path(rpath_tmp2,":;");
	      //_dl_debug_printf("\nRUNPATH Altered Path: %s\n",rpath_tmp2);
	    }
      ...
      else
	{
	  ...
	  if (l->l_info[DT_RPATH])
		...
		{
		  //_dl_debug_printf("\nOriginal Path: %s\n",rpath_tmp2);
		  /* Alter the path to exclude the list in RPATH */
		  ve_alter_search_path(rpath_tmp2,":;");
		  //_dl_debug_printf("\nRPATH Altered Path: %s\n",rpath_tmp2);
		}
	      /* Allocate room for the search path and fill in information
		 from RPATH.  */
  ...
  if (llp != NULL && *llp != '\0')
	...
	{
	  //_dl_debug_printf("\nOriginal Path: %s\n",llp_tmp);
	  /* Alter the path to exclude the list in VE3_LD_LIBRARY_EXCLUDE_PATH */
	  ve_alter_search_path(llp_tmp,":;");
	  //_dl_debug_printf("\nVE_LD_LIBRARY_PATH Altered Path: %s\n",llp_tmp);
	}
...
/* Return error if gap is less than 256B and proceed further */
/* in case of success.                                       */
...
/* @brief To check Flag value in ELF Header */
/* Return error if shared object is generated by old ncc complier. */
static int ve_validate_eflag(const ElfW(Ehdr) *ehdr)
{
  int ret = 0;
  ...
  if(!ve_no_dynamic_cd)
   {
     if(EF_VE_ARCH_VE1 == (ehdr->e_flags & EF_VE_ARCH_MASK) &&
        EF_VE_DYNAMIC_CD == (ehdr->e_flags & EF_VE_DYNAMIC_CD_MASK))
     {
       ret=1;
       goto err_ret;
     }
     if(EF_VE_ARCH_VE3 == (ehdr->e_flags & EF_VE_ARCH_MASK) &&
        EF_VE_DYNAMIC_CD == (ehdr->e_flags & EF_VE_DYNAMIC_CD_MASK))
     {
       ret=1;
       goto err_ret;
			...
			void **stack_endp, Lmid_t nsid)
{
  ...
  /* #1410 Validate when underlying device is VE3 */
...

  {
	...
	case PT_LOAD:
	  ...
	  c->mapstart = ALIGN_DOWN (ph->p_vaddr, p_align);
	  c->mapend = ALIGN_UP (ph->p_vaddr + ph->p_filesz, p_align);
	  c->dataend = ph->p_vaddr + ph->p_filesz;
	  c->allocend = ph->p_vaddr + ph->p_memsz;
	  c->mapoff = ALIGN_DOWN (ph->p_offset, p_align);
  ...
  if (__glibc_unlikely ((stack_flags &~ GL(dl_stack_flags)) & PF_X))
	...
	{
	  const uintptr_t p = (uintptr_t) &__stack_prot & -p_align;
...
# define VALID_ELF_ABIVERSION(osabi,ver) (ver == 0)
  ...
  if (fd != -1)
      ...
      if (__glibc_unlikely (ehdr->e_version != EV_CURRENT))
	{
      ...
      int ret = ve_validate_eflag(ehdr);
      if (__glibc_unlikely (ret != 0))
	{
	  errstring = N_("ELF file Flag value invalid");
	  goto call_lose;
	}
