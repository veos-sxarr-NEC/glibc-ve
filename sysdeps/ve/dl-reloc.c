	...
	if (ph->p_type == PT_LOAD && (ph->p_flags & PF_W) == 0)
	  {
	    ...
	    newp->len = ALIGN_UP (ph->p_vaddr + ph->p_memsz, ph->p_align)
			- ALIGN_DOWN (ph->p_vaddr, ph->p_align);
	    newp->start = PTR_ALIGN_DOWN (ph->p_vaddr, ph->p_align)
			  + (caddr_t) l->l_addr;
