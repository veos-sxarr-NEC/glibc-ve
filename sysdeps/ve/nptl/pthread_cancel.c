...
pthread_cancel (pthread_t th)
{
  ...
  if (INVALID_TD_P (pd))
    ...
    return ESRCH;
#ifndef __ve__
/* pthread_cancel_init() needs "libgcc_s.so.1" to provide force unwinding,
 * In Current vegcc we do not provide "libgcc_s.so.1" library so as a
 * workaround we decided to skip calling pthread_cancel_init()
 *
 * For more details please refer defect refs #1050.
 **/
#ifdef SHARED
  pthread_cancel_init ();
#endif
