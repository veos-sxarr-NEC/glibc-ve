...
free_stacks (size_t limit)
{
  ...
  list_for_each_prev_safe (entry, prev, &stack_cache)
      ...
      if (FREE_P (curr))
	{
	  ...
	  if (__munmap (curr->mem, curr->size) != 0)
	    abort ();
		...
		ALLOCATE_STACK_PARMS)
{
  ...
  if (__glibc_unlikely (attr->flags & ATTR_FLAG_STACKADDR))
      ...
      if (pd == NULL)
	{
	  ...
	  mem = __mmap (NULL, size, PROT_NONE,
...
*/
	...
	size_alloc = ((__static_tls_size) & ~__static_tls_align_m1) + TLS_PRE_TCB_SIZE + __getpagesize();
	...
	pd = (struct pthread *) ((((uintptr_t) map + size_alloc - __static_tls_size)
                                    & ~__static_tls_align_m1)
                                   - TLS_PRE_TCB_SIZE );
#endif
