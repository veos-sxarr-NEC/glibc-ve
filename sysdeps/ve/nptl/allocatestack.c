...
__free_stacks (size_t limit)
{
  ...
  list_for_each_prev_safe (entry, prev, &stack_cache)
      ...
      if (FREE_P (curr))
	{
	  ...
	  stack_cache_actsize -= curr->size;

	  /* Free the memory associated with the ELF TLS.  */
	  _dl_deallocate_tls (TLS_TPADJ (curr), false);

	  /* To deallocate whole stack that was allocated for thread  */
	  if (munmap (curr->mem, curr->size) != 0)
	    abort ();
  ...
  if (__glibc_unlikely (attr->flags & ATTR_FLAG_STACKADDR))
	...
	return EINVAL;
	 ...
	 is wanted.  */
      ...
      pd->size = size; /*Total stack size of thread */
      pd->mem = (char *) attr->stackaddr;/* Storing full map for future reference */
  ...
  else
    {
      ...
      /* Align the size with page size before creating the virtual memory
       * < trac: #1418 > */
      size = (size + (pagesize_m1 - 1)) & - pagesize_m1;
	...
	size += pagesize_m1 + 1;
	 ...
	 eventually the thread descriptor.  */
      ...
      size+=guardsize; /* To include guardsize inside total stack size */
      /* Ref: #1204
         Adding TLS size to size variable, so that when creating virtual memory
	 for a thread, TLS region is also allocated */
      size += ((__static_tls_size) & ~__static_tls_align_m1) + TLS_PRE_TCB_SIZE;
      size = (size + (pagesize_m1 - 1)) & - pagesize_m1;
...
*/
	size_t size_alloc = 0;
	unsigned char *map = 0, *stack_start = 0;
        /* Update size_alloc for calculating thread descriptor */
	size_alloc = ((coloring + __static_tls_size) & ~__static_tls_align_m1) + TLS_PRE_TCB_SIZE + __getpagesize();
	size_alloc = (size_alloc + (pagesize_m1 - 1)) & - pagesize_m1;
	stack_start = (mem + size) - size_alloc;
	map = mmap(stack_start, size_alloc, prot,
                                        MAP_PRIVATE|MAP_ANON|MAP_FIXED|MAP_STACK, -1, 0);

	if (__glibc_unlikely (map == MAP_FAILED))
		return errno;

	/* Calculate pd using size_alloc */
	pd = (struct pthread *) ((((uintptr_t) map + size_alloc - coloring
                                    - __static_tls_size)
                                    & ~__static_tls_align_m1)
                                   - TLS_PRE_TCB_SIZE );
...
#endif
	  /* Remember the stack-related values.  */
	  pd->stackblock = mem; /* Update Stack Pointer */
	  pd->stackblock_size = size;/*Calculate stack block size */
          pd->mem = mem; /* Storing full map for future reference */
          pd->size = size; /* Total thread size */
