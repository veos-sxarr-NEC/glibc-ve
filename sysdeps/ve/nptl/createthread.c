...
#define THREAD_CREATION_FAILURE -1
...
/* Error Code handling  required for __clone */
  int clone_ret = 0;
  if ((clone_ret =__glibc_unlikely (ARCH_CLONE (&start_thread, STACK_VARIABLES_ARGS,
				    clone_flags, pd, &pd->tid, tp, &pd->tid, guard_ptr)
			  )) < 0)
     return -clone_ret;
