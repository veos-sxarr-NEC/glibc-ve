...
#if !defined __ve__
/* Current we are supporting __libc_unwind_longjmp() for unwinding. */
...
__pthread_unwind (__pthread_unwind_buf_t *buf)
{
   ...
   *
   * For force unwind support this function is entry point, adopting
   * __libc_unwind_longjmp() as a substitute ensures that  "libgcc_s.so.1" is
   * no more required and is error free.
   *
   * Please refer defect refs #1050 for more details.
   * */
