	   ...
	   unsigned int flags)
{
  ...
  if (prof_info.num_regions > 0)
    {
      /* Disable profiling.  */
      if (__setitimer (ITIMER_REAL, &prof_info.saved_timer, NULL) < 0)
	return -1;

      if (__sigaction (SIGALRM, &prof_info.saved_action, NULL) < 0)
	return -1;
  ...
  if (__sigaction (SIGALRM, &act, &prof_info.saved_action) < 0)
    return -1;
  ...
  return __setitimer (ITIMER_REAL, &timer, &prof_info.saved_timer);
}
