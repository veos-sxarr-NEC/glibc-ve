...
# define otimer_ptr &otimer
  ...
  if (sample_buffer == NULL)
	...
	return 0;
      ...
      if (__setitimer (ITIMER_REAL, &otimer, NULL) < 0)
	return -1;
      samples = NULL;
      return __sigaction (SIGALRM, &oact, NULL);
    }
 ...
 if (samples)
    {
	 ...
	 first.  */
      if (__setitimer (ITIMER_REAL, &otimer, NULL) < 0
	  || __sigaction (SIGALRM, &oact, NULL) < 0)
	return -1;
    }
#else
...
# define otimer_ptr NULL
  ...
  if (__sigaction (SIGALRM, &act, oact_ptr) < 0)
    return -1;

  timer.it_value.tv_sec = 0;
  timer.it_value.tv_usec = 1000000 / __profile_frequency ();
  timer.it_interval = timer.it_value;
  return __setitimer (ITIMER_REAL, &timer, otimer_ptr);
}
